...
class VideoFetcher {
	...
	async updateChannelsList() {
		...
		const youtubeAccount = [...new Set(data.map(item => item.Youtube.Account))];

		// Put subreddits with their list of channels to post to
		this.channels = youtubeAccount.map(sub => ({ subredditName: sub, channelIDs: data.filter(({ Youtube }) => Youtube.channelID === sub) }));
	}
